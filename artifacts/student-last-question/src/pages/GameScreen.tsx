...
export default function GameScreen() {
  ...
  const [damageNumbers, setDamageNumbers]     = useState<
    Array<{ id: string; amount: number; isPlayer: boolean; isHeal?: boolean }>
  >([]);
  ...
  // ─── Mutable refs (latest values accessible inside async callbacks) ──────────
  ...
  useEffect(() => {
    ...
    loadNewQuestion(0, selectedSubject);
  ...
  const triggerUltimate = useCallback(() => {
    // Hard guards: not full, already answering, or already fired this frame
    if (ultimateGauge < ULTIMATE_MAX || selectedAnswer !== null || ultimateUsedRef.current) return;

    ultimateUsedRef.current = true;          // block any re-entry immediately
    setUltimateGauge(0);                     // reset gauge → button exits NOW (isFull = false)

    const baseDamage = Math.floor(Math.random() * 11) + 20;
    const damage     = baseDamage * 2;       // 2× damage
    const newEnemyHP = Math.max(0, enemyHPRef.current - damage);
    enemyHPRef.current = newEnemyHP;
    setEnemyHP(newEnemyHP);
    setEnemyHit(true);
    setFeedback(`⚡ ULTIMATE! โจมตี 2× = ${damage} ความเสียหาย! เกจรีเซ็ตเป็น 0%`);
    setDamageNumbers(prev => [
      ...prev, { id: `ult-${Math.random()}`, amount: damage, isPlayer: false },
    ]);
    setTimeout(() => { setEnemyHit(false); ultimateUsedRef.current = false; }, 500);

    if (newEnemyHP <= 0) {
      setTimeout(() => handleEnemyDefeated(), 1500);
    }
  }, [ultimateGauge, selectedAnswer]);   // handleEnemyDefeated excluded intentionally (stable ref)

  // ─── Stage cleared / enemy defeated ─────────────────────────────────────────
  const handleEnemyDefeated = useCallback(() => {
    ...
    setStageClearedText(isBoss ? "เอาชนะบอสได้แล้ว!" : `ผ่านด่านที่ ${cleared} แล้ว!`);
    ...
    setTimeout(() => {
      ...
      if (nextIdx >= allEnemies.length) {
        ...
        // ── Normal stage cleared → open shop ──
  ...
  const handleShopClose = useCallback(() => {
    setShowShop(false);
    if (pendingNextStage) advanceStage(stageIndexRef.current + 1);
  ...
  const handleShopBuy = useCallback((itemId: string) => {
    ...
    if (itemId === "potion" && currentCoins >= 30 && playerHPRef.current < 100) {
      setPotionCount(prev => {
        ...
        setDamageNumbers(p => [
          ...p, { id: `heal-${Math.random()}`, amount: healed, isPlayer: true, isHeal: true },
        ]);
        return prev + 1;
      });
  ...
  const handleAnswer = (index: number) => {
    if (selectedAnswer !== null || !currentQuestion || showShop) return;
    ...
    if (isCorrect) {
      ...
      if (buffCount > 0) { damage += 20; setBuffCount(prev => prev - 1); }
      ...
      // Ultimate gauge: +20% per correct answer
      ...
      setDamageNumbers(prev => [
        ...prev, { id: Math.random().toString(), amount: damage, isPlayer: false },
      ]);
      setTimeout(() => setEnemyHit(false), 500);

      if (newEnemyHP <= 0) {
        // Monster defeated → advance stage (do NOT load next question)
      ...
      if (shieldActive) {
        ...
        setDamageNumbers(prev => [
          ...prev, { id: Math.random().toString(), amount: damage, isPlayer: true },
        ]);
    ...
    // Monster still alive (or wrong answer, player alive) → load next question automatically
    setTimeout(() => {
      setTurn(prev => prev + 1);
      if (selectedSubject) loadNewQuestion(stageIndexRef.current, selectedSubject);
    }, 1500);
  };
  ...
  if (!currentQuestion || !selectedCharacter || !selectedSubject) return null;
    ...
    <div className="min-h-[100dvh] w-full flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col h-full min-h-[100dvh] p-2 md:p-4">
        ...
        <div className="flex items-center justify-between mb-1 gap-2 flex-wrap">
          <div className="flex items-center gap-2">
            <span
              ...
              className={`text-[9px] px-2 py-0.5 border-2 border-white text-white ${
                currentEnemy.isBoss ? "bg-destructive" : "bg-secondary"
              }`}
            >
              {stageLabel}
            </span>
          ...
          <span
            className="text-[9px] text-yellow-400 border border-yellow-600 px-2 py-0.5"
        ...
        <div className="flex justify-between items-start gap-2 mb-1">
          <HPBar hp={playerHP} maxHP={100} name={selectedCharacter.thaiName} isPlayer />
          <HPBar hp={enemyHP} maxHP={currentEnemy.maxHP} name={currentEnemy.name} isBoss={currentEnemy.isBoss} />
        </div>
        ...
        <div className="flex justify-center mb-1">
          <UltimateGauge
            ...
            disabled={selectedAnswer !== null || showShop || ultimateUsedRef.current}
          />
        </div>
        ...
        <div className="flex-1 flex justify-between items-end px-2 md:px-8 relative min-h-[140px] md:min-h-[220px]">
          ...
          <div className="relative flex flex-col items-center">
            ...
            {damageNumbers.filter(d => d.isPlayer).map(d => (
              <DamageNumber key={d.id} id={d.id} amount={d.amount} x={30} y={-20}
                onComplete={removeDamageNumber} isHeal={d.isHeal} />
            ))}
          </div>
          ...
          <div className="relative flex flex-col items-center">
            <motion.div
              animate={enemyHit
                ? { x: [8, -8, 8, -8, 0], filter: ["brightness(3)", "brightness(1)"] }
                : {}}
              transition={{ duration: 0.4 }}
            >
              <img
                ...
                className={`h-28 md:h-48 w-auto object-contain object-bottom ${
                  currentEnemy.isBoss
                    ? "drop-shadow-[0_0_20px_rgba(239,68,68,0.9)]"
                    : "drop-shadow-[0_0_12px_rgba(139,92,246,0.8)]"
                }`}
